...
//headless ui
import { Menu, menu } from '@headlessui/react'
...
//location data
const locations = [
  "7 No North Nowadapara, West Bengal",
        ...
        <Menu.Items className='dropdown-menu shadow-lg absolute -top-56 xl:top-[90px] left-1/2 xl:left-0 z-10 transform -translate-x-1/2 xl:-translate-x-0 text-sm text-center xl:text-left w-full bg-white max-w-[332px] py-6 rounded-[10px]'>
          {
            locations.map((location, index)=>{
              ...
              key={index} 
              className='cursor-pointer py-4 xl:pl-10 hover:bg-gray-50 text-[13px] uppercase'>{location}</div>
            })
          }
        </Menu.Items>
      </div>
